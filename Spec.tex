...
\textbf{Issue:} this is currently the simplest possible kind of Substrate; it represents the DNF form of SNOMED~CT with concepts only
(i.e., \textit{Expressions} and \textit{Expression Libraries} are not supported).

\textbf{Issue:} this is a \textit{flat} representation (fixed to 1 level of nesting corresponding to role grouping) and does not properly handle Concrete Domains (which almost always involve additional nesting
...
\item The descendants function, $descs$, is the  \emph{irreflexive} transitive closure of the inverse of the $parentOf$ relationship. 
