...
\begin{gendef}
   ...
   i\_crOrWildCard : Substrate \fun crOrWildCard \fun Sctids\_or\_Error
\where
   \forall ss : Substrate; fc : focusConcept @ \\
i\_focusConcept~ss~fc = \\
\t1 \IF focusConcept\_c \inv fc \in crOrWildCard \THEN \\
\t2 i\_crOrWildCard~ss~(focusConcept\_c~\inv fc) \\
\t1 \ELSE \\
\t2  i\_memberOf~ss~(i\_crOrWildCard~ss~(focusConcept\_m~\inv fc)) 
\also
  \forall ss : Substrate; crorwc : crOrWildCard @ \\
i\_crOrWildCard~ss~crorwc = \\
\t1 \IF cr \inv crorwc \in conceptReference \THEN \\
