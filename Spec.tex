...
\item The $childOf$ relationship is defined as the function from a concept in $c$ to the set of concepts that are the target of $c$ in an $is\_a$ relationship in the $zero\_group$
\item The $parentOf$ relationship is defined as a function from a concept $c$ to the set of concepts that are the source of of $c$ in an $is\_a$ relationship in the $zero\_group$
\item The $parentOf$ and $childOf$ relationships are irreflexive.
\item The ancestors function, $ancs$ is the \emph{irreflexive} transitive closure of the $childOf$ relationship.
\item The descendants, $descs$ is the  \emph{irreflexive} transitive closure of the inverse of the $parentOf$ relationship. 
\item The reference set function is a function from  \emph{subset of} $c$ to set of SCTID's in $c$.  \req{An SCTID that is not in the domain of $rs$ cannot appear as the target of a memberOf function}
\end{enumerate}
...
compoundEntry \defs [op: compoundOperator; cons: simpleExpressionConstraint] \\
